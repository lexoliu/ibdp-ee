...
% Author omitted for IB Extended Essay anonymous assessment
...
\begin{itemize}
    \item \textbf{Service stability under extended load}: All implementations experienced service restarts during 10-minute benchmark runs (Prime/Light workloads at 300-450s), limiting measurements to stable operation periods. Root cause investigation was beyond scope due to resource constraints; however, 300-400 second stable windows exceed typical microservice request lifecycles and provide sufficient data for comparative analysis.

    \item \textbf{Measurement duration}: 10-minute runs may not reveal long-term GC behavior such as memory fragmentation or heap growth patterns that emerge over hours of operation.

    \item \textbf{Workload representativeness}: Synthetic workloads, while controlled, may not capture the full complexity of production applications with diverse request patterns and external dependencies.

    \item \textbf{Single-environment testing}: Results from Aliyun Tokyo region may not generalize to other cloud providers or hardware configurations.

    \item \textbf{Memory measurement granularity}: RSS measurements include shared library pages and allocator overhead, potentially obscuring true application memory footprint.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Efficient memory management across strategies}: All languages maintained remarkably low and stable memory footprints (0.9--1.0GB for 6.4M key-value pairs), indicating that both manual and automatic memory management approaches can be highly efficient in practice.
